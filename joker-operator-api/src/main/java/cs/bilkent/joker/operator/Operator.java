 ...
 * <p></p>
 * {@link Operator#init(InitCtx)} and {@link Operator#shutdown()} methods are defined to allow developers to
...
public interface Operator
{
     ...
     * and objects that can be used during lifetime of an operator is provided via the {@link InitCtx} interface.
     * Operators can save references of the objects returned by the {@link InitCtx}, i.e. {@link OperatorConfig},
     ...
     * create files, connect to some external services etc. Please keep in mind that {@link Operator#init(InitCtx)} method
     * is going to be invoked for every instance of a given operator implementation.
     * Please see {@link InitCtx} for details.
     * <p>
     ...
     */
    SchedulingStrategy init ( InitCtx ctx );
     ...
     * {@link #invoke(InvocationCtx)} method is given in the {@link InvocationCtx} object. Please keep in mind that
     * the objects accessed via {@link InvocationCtx} should not be cached by the operator between multiple invocations, and should be
     * used only within the current invocation.
     * <p>
     * Tuples sent to the incoming ports of an operator are accessed via the methods on {@link InvocationCtx}.
     * Similarly, tuples produced during an invocation can be passed to the downstream by using the methods on {@link InvocationCtx}.
     * <p>
     ...
     * If it is {@code false}, there will be no more invocations and all of the tuples provided with the {@link InvocationCtx}
     ...
     * implementation is provided with the {@link InvocationCtx#getKVStore()} method. Additionally, the runtime provides
     ...
     * <ul>
     * <li>{@link Tuple} objects returned by input methods of {@link InvocationCtx} should be considered final and should not be
     * modified. It is because the same {@link Tuple} objects can be processed by other operators in the flow.</li>
     * <li>{@link Tuple} objects added to the {@link InvocationCtx} via output methods should not be modified.</li>
     * </ul>
     *
     * @param ctx
